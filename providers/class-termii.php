...
class Termii extends Provider_Base {
    ...
    public function get_settings_fields(): array {
        return [
            ...
            [ 'key' => 'channel',   'label' => 'Channel',    'type' => 'select',   'required' => false,
              'options' => [ 'generic' => 'Generic', 'dnd' => 'DND', 'WhatsApp' => 'WhatsApp', 'email' => 'Email' ] ],
        ];
    }

    public function send( string $to, string $message, array $args = [] ): array {
        $s = $this->settings();
        $r = $this->http_post_json( 'https://api.ng.termii.com/api/sms/send', [
            'to'      => $to,
    ...
    public function get_balance(): array {
        $s = $this->settings();
        $r = $this->http_get( 'https://api.ng.termii.com/api/get-balance', [ 'api_key' => $s['api_key'] ?? '' ] );
